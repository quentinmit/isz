  ...
  for device in os.listdir(ROOT):
    ...
    for de in os.scandir(os.path.join(ROOT, device)):
      ...
      if "energy_uj" in result:
        ...
        result["energy_uj_cumulative"] = last_result.get("energy_uj_cumulative", 0) + (energy_uj - last_energy_uj)
        if result["energy_uj_cumulative"] > 0x7fffffffffffffff:
            # Wrap at 2^63
            result["energy_uj_cumulative"] = 0
    print("intel_rapl,device=%s %s %d" % (device, ",".join("%s=%di" % (k,v) for k,v in result.items() if v), int(time.time()*1e9)))
