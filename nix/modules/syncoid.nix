...
let
  ...
  dedupeNames = l: lib.pipe l [
    (lib.groupBy (e: e.name))
    (lib.mapAttrs (name: elements:
      if (builtins.length elements) == 1 then elements else lib.imap1 (i: e: e // {
  ...
  allowCommands = lib.pipe cfg.commands [
    ...
    (lib.mapAttrsToList (command: c:
      (lib.concatMap (dataset: lib.map (permission: {
        inherit command dataset permission;
      }) c.localSourceAllow) (localDatasetName c.source))
      ++ (lib.concatMap (dataset: lib.map (permission: {
        inherit command dataset permission;
      }) c.localTargetAllow) (localDatasetName c.target))
    ))
    lib.concatLists
    # Group commands by (dataset, permission)
    (lib.groupBy (p: "${p.dataset}@${p.permission}"))
    (lib.mapAttrsToList (_: ps: {
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services = allowUnits // lib.mapAttrs' (
      name: c: let
        commandAllowUnits = allowUnitsByCommand.${name};
      in lib.nameValuePair "syncoid-${escapeUnitName name}" (
        lib.mkMerge [
          {
